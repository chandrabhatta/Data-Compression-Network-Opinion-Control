...
def test_controllability(A, B):
    # Check Hautus criteria given A and B matrices
    ...
    all_controllable = all(matrix_rank(np.hstack([A - λ * I, B])) == N for λ in eig_A)
...
def optimize_input(A, x0, xf, s, K, u_max=None, method='L1'):
    ...
    # Build controllability matrix --> can also update H recursively to save memory and computation time
    ...
    if method == 'L1' or 'L2' or 'LASSO' or 'RIDGE':
        ...
        U = cp.Variable(N * K)
        objective = obj_selector(method, H, U, b, mu=0.1) # Entrywise L1
        # minimize l0/1 norm of u (residue: r = Ax-u) --> approx l0 as l1 for convexity
        # objective_lasso = cp.Minimize(cp.norm2(A @ x - u) + mu*cp.norm1(u)) # LASSO problem formulation with mu=regularizer
        # TODO --> add function parameter to select the objective function: {BP, LASSO, Ridge, etc.} --> maybe for greedy algorithms make seperate function
        ...
        constraints = [H @ U == b]
        # Add bound constraints if specified
        ...
        opt_cost = problem.solve(solver=cp.OSQP)
