...
def matching_persuit(H, b, max_iter=2):
    """
...
def orthogonal_matching_persuit(H, b, s):
    ...
    for _ in range(s): # Need s iterations for s-sparse recovery generally
        ...
        H_selected = H[:, supp]
        u_ls = np.linalg.pinv(H_selected) @ b # LS on the support
        u_supp.append(u_ls) # store the current LS coefficient along with the support
        # x_supp = np.linalg.lstsq(H[:, supp], b, rcond=None)[0] # can use lstsq function alternatively

        res = b - H_selected @ u_supp # corrected dimensions
...
def main():
    ...
    s = 10
    K = 25
    optimize_input(A,x0,xf,s,K, method ='MP')
